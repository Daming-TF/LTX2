...
import torch
import pdb
tmp = [torch.randn(1,257)]*4
print(torch.concat(tmp).shape)
# attention_mask_binary = torch.randn(4,1024,1)
# print(attention_mask_binary.unsqueeze(1).shape)
# hidden_states = torch.randn(4,1024,3840)
# # pdb.set_trace()
# # non_zero_hidden_states = hidden_states[:, attention_mask_binary.squeeze().bool(), :]
# non_zero_hidden_states = torch.stack([
#     hidden_states[i, mask, :] 
#     for i, mask in enumerate(attention_mask_binary.squeeze(-1).bool())
# ])
# # pdb.set_trace()
# print("successful")
# print(non_zero_hidden_states.shape)
